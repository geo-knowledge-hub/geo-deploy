...
# This makes GKH_API_TOKEN and GKH_BASE_URL available as environment
# variables so you don't have to pass --api-token on every run.
#
# Priority order (highest to lowest):
#   1. CLI flag         --api-token "xyz"   (always wins)
#   2. Shell env var    $env:GKH_API_TOKEN  (already set in the shell)
#   3. .env file        GKH_API_TOKEN=xyz   (loaded here)
#
...
def pytest_addoption(parser: pytest.Parser) -> None:
    parser.addoption(
        "--base-url",
        default=os.getenv("GKH_BASE_URL", ""),
        help="Base URL of the GEO Knowledge Hub instance (or set GKH_BASE_URL in .env)",
    )
    parser.addoption(
        "--api-token",
        default=None,
        help="Bearer token (or set GKH_API_TOKEN in .env)",
    )
    parser.addoption(
        "--no-verify-tls",
        action="store_true",
        default=os.getenv("GKH_NO_VERIFY_TLS", "false").lower() == "true",
        help="Disable TLS verification (or set GKH_NO_VERIFY_TLS=true in .env)",
    )
