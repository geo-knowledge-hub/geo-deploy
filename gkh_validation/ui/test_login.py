...
class TestLoginPage:
    ...
    def test_login_page_returns_200(self, base_url: str, verify_tls: bool) -> None:
        r = requests.get(f"{base_url}/login", verify=verify_tls, timeout=15, allow_redirects=True)
    ...
    def test_logout_does_not_crash(self, base_url: str, verify_tls: bool) -> None:
        r = requests.get(f"{base_url}/logout", verify=verify_tls, timeout=10, allow_redirects=True)
    ...
    def test_account_page_accessible(self, base_url: str, verify_tls: bool) -> None:
        ...
        r = requests.get(f"{base_url}/account", verify=verify_tls, timeout=10, allow_redirects=True)
        assert r.status_code < 500
